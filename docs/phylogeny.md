...
## Ejecución local consecutiva (v0.3)

Para una evaluación local completa de paneles de 10 genes:

```bash
phylomarker-phylogeny local-run --config phylogeny.local.10genes.yaml
```
...
1. validación;
2. preparación;
3. árboles génicos con ModelFinder;
4. extracción de modelos;
5. concatenación particionada;
6. ASTRAL;
7. gCF/sCF;
8. RF concatenación–ASTRAL con DendroPy;
9. jackknife;
10. resumen del jackknife y estado final.

La ejecución es reanudable: si un `.treefile`, `.cf.tree` o árbol ASTRAL ya existe
y no está vacío, se omite automáticamente.

Resultados nuevos:

```text
summaries/tree_comparisons.tsv
summaries/jackknife_replicates.tsv
summaries/jackknife_summary.tsv
summaries/pipeline_status.tsv
```
